...
pub use common::{ZCanChlMode, ZCanChlStatus, ZCanChlType};
...
impl TryFrom<&HashMap<String, u32>> for ZCanFdChlCfgSet {
    ...
    fn try_from(value: &HashMap<String, u32>) -> Result<Self, Self::Error> {
        let &tseg1 = value.get(TSEG1).ok_or(CanError::OtherError(format!(
            "`{}` is not configured in file!",
            TSEG1
        )))?;
        let &tseg2 = value.get(TSEG2).ok_or(CanError::OtherError(format!(
            "ZLGCAN - `{}` is not configured in file!",
            TSEG2
        )))?;
        let &sjw = value.get(SJW).ok_or(CanError::OtherError(format!(
            "ZLGCAN - `{}` is not configured in file!",
            SJW
        )))?;
        let &smp = value.get(SMP).ok_or(CanError::OtherError(format!(
            "ZLGCAN - `{}` is not configured in file!",
            SMP
        )))?;
        let &brp = value.get(BRP).ok_or(CanError::OtherError(format!(
            "ZLGCAN - `{}` is not configured in file!",
            BRP
        )))?;
...
pub(crate) fn get_fd_set(
    ...
    let aset = bitrate_ctx
        ...
        .ok_or(CanError::OtherError(format!(
            "bitrate `{}` is not configured in file!",
            bitrate
        )))?;
    let dset = match dbitrate {
        Some(v) => {
            // dbitrate is not None
            match dbitrate_ctx {
                Some(ctx) => {
                    // dbitrate context is not None
                    match ctx.get(&v.to_string()) {
                        Some(value) => Ok(value),
                        None => Err(CanError::OtherError(format!(
                            "data bitrate `{}` is not configured in file!",
                            v
                        ))),
                    }
                }
                None => {
                    // dbitrate context is None
        ...
        None => {
            // dbitrate is None
            match dbitrate_ctx {
                Some(ctx) => match ctx.get(&bitrate.to_string()) {
                    Some(value) => Ok(value),
                    None => Ok(aset),
                },
                None => Ok(aset),
